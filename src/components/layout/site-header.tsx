...
export function SiteHeader() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Show header when scrolled past 80vh
  ...
  return (
    <header
      className="border-border/30 fixed top-0 left-0 right-0 z-50 border-b bg-[color:var(--header-bg)] py-4 shadow-[0_8px_32px_rgba(0,0,0,0.12)] backdrop-blur-2xl transition-all duration-300 ease-out"
      style={{
    ...
    >
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-8">
        ...
        >
          <span className="text-accent block text-base font-bold uppercase tracking-[0.5em] transition-opacity duration-300 group-hover:opacity-80">
            LUV
          </span>
